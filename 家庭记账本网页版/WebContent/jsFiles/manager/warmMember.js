function TurnToWarmMember()
{
	...
	Area += "	<br>";
	Area += "	<br>";
	Area += "	<button onclick='CheckAndWarmMember()' class='basicButton'>确认解冻</button>";
	Area += "</center>";
	document.getElementById("RightArea").innerHTML = Area;
	document.getElementById("name").focus();
}
function CheckAndWarmMember()
{
			...
			if (xmlHttp.readyState == 4) {
			    if (xmlHttp.status == 200) 
			    {
		    		...
		    		if(InformationSet[0].Length==0)
			    		alert("用户不存在！");
		    		else if(InformationSet[1].Isfrozen=="0")
		    			alert("该用户为非冻结账号！");
		    		else
		    			WarmMember(InformationSet[1].Code,InformationSet[1].Name,InformationSet[1].Password);
		    	}
		   		else 
...
function WarmMember(code,name,password)
{
	var xmlHttp = null;
	try{
		xmlHttp = new XMLHttpRequest();
